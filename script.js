...
//modalIsOpen, modalContentImgs, menuGridCards, this, e, subMenuGridCards, linkPictures, linkGithub, targetGridCards

function quitMenuGridCardsClickWindow(this) {
    console.log(this);
    // if (modalIsOpen === 1) {
...
let oldCard;
let oldMenuGridCards;

gridCards.forEach(item => {
    item.addEventListener('click', function(e) {
        e.preventDefault();
        if (oldMenuGridCards !== undefined) {
            oldMenuGridCards.replaceWith(oldCard);
        }
        oldCard = this;
        // conteneur
        let menuGridCards = document.createElement('div');
        oldMenuGridCards = menuGridCards;
        menuGridCards.classList.add('menu-grid-cards');
        this.parentNode.insertBefore(menuGridCards, this);
        menuGridCards.appendChild(this);
        // sous conteneur
        let subMenuGridCards = document.createElement('div');
        ...
        // images
        let linkPictures = document.createElement('a');
        linkPictures.href = '#';
        ...
        //let clickCard = 0;
        //let clickWindow = 0;
        const targetGridCards = e.target;
        ...
        window.addEventListener('click', quitMenuGridCardsClickWindow(menuGridCards), {capture: true});
